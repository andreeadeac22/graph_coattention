...
def preprocess_decagon(dir_path='./data/'):
	...
	atom_attr_keys = {a_key for d in raw_drugs.values() for a in d['atoms'] for a_key in a.keys()}
	print('Possible atom attribute names:', atom_attr_keys)
	bond_attr_keys = {b_key for d in raw_drugs.values() for b in d['bonds'] for b_key in b.keys()}
	...
	def collate_molecule(molecule, self_loop=True):
		...
		bonds = {  # symmetric bonds
			aid_pair: '{}-{}'.format(b['order'], b.get('style', 0)) for b in molecule['bonds']
			for aid_pair in [(b['aid1'], b['aid2']), (b['aid2'], b['aid1'])]}
		...
		bonds = {
			aid: [(b_aid2, b) for (b_aid1, b_aid2), b in bonds.items()
			      if aid == b_aid1] for aid in atoms}

		# Hydrogen bookkeeping
		h_aid_set = {aid for aid, atom in atoms.items() if atom['number'] == 1}
		h_count_dict = {
			aid: sum(int(nbr in h_aid_set) for nbr, _ in bonds[aid])
			for aid, _ in atoms.items() if aid not in h_aid_set}
		...
		atoms_wo_h_new_aid = {
			aid: {**a, 'charge': a.get('charge', 0), 'n_hydro': h_count_dict.get(aid, 0), 'aid': idx}
	...
	drug_structure_dict = {cid: collate_molecule(d, self_loop=True) for cid, d in raw_drugs.items()}
	...
	bond_types = {b for d in drug_structure_dict.values() for a in d for _, b in a['nbr']}
