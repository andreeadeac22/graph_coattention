...
def read_drug_structure(drug_feat_idx_jsonl):
	with open(drug_feat_idx_jsonl) as f:
		drugs = [l.split('\t') for l in f]
		drugs = {idx: json.loads(mol) for idx, mol in tqdm(drugs)}
	return drugs
...
def prepare_decagon_cv_files(opt):
	...
	opt.drug_dict = read_drug_structure(opt.drug_data)
...
def main():
	...
	print('Dump to file:', opt.output_npy)
	np.save(opt.output_npy, opt)
